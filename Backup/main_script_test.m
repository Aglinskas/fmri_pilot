...
%% Scanner at start initialize port code
%killserial
...
param_ISI = 1; % InterStimulusInterval: 1=Fixed ; 2 = Random between min and max values ; 3 optseq2 (not added yet)
    param_fixedISI = 2; % if fixed specify seconds     %default 2 seconds, for debugging 0.1s 
...
%Task{14,1} = 'Quanti anni avevi quando hai sentito parlare di questa persona per la prima volta?'; %episodic
%Task{14,2} = '1 = Conosco questa persona da sempre\n2 = Ero un adolescente\n3 = In età adulta\n4 = Non ho mai visto questa persona';
...
%% Set up InterStimulusInterval: not needed anymore
%%%
...
%rf_block = ;
% rf_block = 1:75;
%% scanner Shuffle([1:myTrials(length(myTrials)).fmriblock])'trigger
