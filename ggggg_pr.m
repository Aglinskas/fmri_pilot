...
sca; %        
%% SUBJECT ID
subjID = 'New_Pilot_test'%'New_Pilot_subject_15'
%% get the practise (first half)
myTrials = func_myPracticeTrials(7,1); %ins = 2, gives english instructions, 1= italian
%% parameters
numBlocks = 12; % how many blocks to run       azmkakkamzakzmakzmakzmZKamzkammakzmazkazmakzmaz  azaakak  kkzkaaa  aaxzkmam  zakaa  mkazam  kzama  zmkmkaamammz  kzakk  maaakaa  azazazazazazazazazzazaza  aazazz azkmkmkkakazaaaaakkakazzaaazaz in experiment if 15 = all blocks will be presented in a random order, if less, a random subset of tasks will be selected
...
elseif pace == 3 % 
   ...
   while GetSecs < t_fix + rsps_time || isDown == 0
    RestrictKeysForKbCheck([4;29;16;14]);
    while isDown == 0
   [isDown secs button_press c] = KbCheck(-1);
    ...
    if strmatch(myTrials(ExpTrial).response{1},'a')
       rspns = 1;
    elseif strmatch(myTrials(ExpTrial).response{1},'z')
        rspns = 2;
    elseif strmatch(myTrials(ExpTrial).response{1},'m')
        rspns = 3;
    elseif strmatch(myTrials(ExpTrial).response{1},'k')
        rspns = 4;
    end
...
%% text doesnt work well with Italian istructions
% text = 'Bel lavoro! Le domande dovrebbero essere pi? chiare ora.\nNella prossima sezione troverai le stesse domande,\nma il tutto sar? un po? pi? difficile:\ndovrai ricordare le opzioni di ?risposta a scelta?\nquando ti verranno presentate\nall?inizio del blocco poich? non\nti verranno mostrate\npi? alla fine di ogni volto. Cerca di essere veloce ed accurato'
% Screen('TextSize', window, 28);
% Screen('TextFont', window, 'Courier');
...
% KbWait(-1)
%% bell lavoro
theImageLocation = 'Other/bell_lavoro.001.jpeg';
theImage = imread(theImageLocation);
imageTexture = Screen('MakeTexture', window, theImage);
Screen('DrawTexture', window, imageTexture, [], windowRect,0)
Screen('Flip', window);
